...
(function ($) {
    ...
    $.fn.lightGallery = function (options) {
        ...
        var setUp = {
            ...
            structure: function () {
                $('body').append('<div id="lightGallery-outer" class="' + settings.addClass + '"><div id="lightGallery-Gallery"><div id="lightGallery-slider"></div><a id="lightGallery-close" class="close"></a> ' + (settings.enableRemove ? ' <a id="lightGallery-remove" class="remove"></a>' : '') + '</div></div>').addClass('lightGallery');
            ...
            closeSlide: function () {
                ...
                $('#lightGallery-remove').bind('click touchend', function () {
                    $this.remove();
            ...
            remove: function () {
                var $this = this;
                var index = $gallery.find('.thumb.active').index()

                var result = true;
                if (settings.removeHandler != null && typeof (settings.removeHandler) == "function") {
                    var result = settings.removeHandler($gallery.find('.current .object').attr('src'));
                }
                if (result != false) {
                    plugin.children().eq(index).remove();
                    if (plugin.children().length > 0) {
                        quickClose = true;
                        plugin.destroy();
                        plugin.lightGallery(settings);
                        plugin.eq(0).find('li:first').click();
                        quickClose = false;
            ...
            mouseWheel: function () {
                ...
                if ($.fn.mousewheel != undefined) {
                    $(window).bind('mousewheel.lightGallery', function (event, delta) {
        ...
        plugin.destroy = function (d) {
            ...
            $(window).unbind('mousewheel.lightGallery');
