...
function OnWebTwainNotFoundOnMacCallback(ProductName, InstallerUrl) {
    ...
    var iStartPos = ua.indexOf("chrome/") + 7;
    var iEndPos = ua.indexOf(".", iStartPos);
    var version = ua.substring(iStartPos, iEndPos);
    if (parseInt(version) >= 39) {
        var ObjString = [
        '<p style="height:1px;"/>',
		'<p>Currently 64bit Chrome is not supported on Mac, please use Safari (32bit) or Firefox (32bit) instead. 64bit browsers will be supported in our next official big release. Stay tuned.</p>'
		];

        Dynamsoft.WebTwainEnv.ShowDialog(392, 180, ObjString.join(''));
    }
    else {
